...
std::string parse_Spec (std::string s) {
    ...
    while (i < s.length() ) {
        ...
        while ( s[i] != '-') {
            ...
            std::string property_Spec =  s.substr( i , s.find("..", i) - i );   
            ...
            else if (section_type == "GP" || section_type == "OTP" ) { // 
                if (!spec_flag) {
...
std::string trivial_modifications (std::string s) {
    ...
    s = replace_all_substr(s, "Ordered Tree Property", "OTP" );
    s = replace_all_substr(s, "Attributes", "AG" );
    s = replace_all_substr(s, "Graph Property", "GP" );
    s = replace_all_substr(s, "Specification Conditions:", "%" );
    ...
    s = add_end_delimiter(s , "AG");
    s = add_end_delimiter(s , "GP");
...
void initialize_operators( std::map<string, long>& infix_operator_precedence, std::set<string>& prefix_operators) {
    ...
    prefix_operators.insert("is_set"); 
    ...
    while ( !operator_stack.empty() ) {
        ...
        while (!operator_stack_auxiliary.empty()) {
            ...
            if (operator_temp == "=" || operator_temp == "in")
                operator_temp = type_inference(operator_temp, temp_string, operand_temp);     // TYPE INFERENCE GIVES SEGFAULT - FIX HERE!
...
string read_next_word (string s, long start_pos, long& end_pos) {
    ...
    if (s[j] == '(') 
        j++, ob++;
    while ( j < s.size() && !(isWS(s[j]) && ob == 0 && sb == 0) && !(s[j] == '(' && ob == 0 && sb == 0)  ) {  // The open brace case is to treat arguments to a prefix operator as a new word
        if (s[j] == '(')
            ob++;
        else if (s[j] == ')')
            ob--;
...
// // Need to use stack for this; and for this need to call infix_to_prefix_expression!
// std::string infix_to_prefix_function (std::string s, std::string infix_string, std::string prefix_string) {

//     long search_start = 0;
//     long previous_index = -1;
//     long current_index = -1;
//     size_t end_delimiter_size = sizeof(END_DELIMITER) - 1;
//     while ( s.find( END_DELIMITER , search_start) != std::string::npos ) {
//         previous_index = current_index;
//         current_index = s.find( END_DELIMITER , search_start);
//         std::string section = s.substr(previous_index + end_delimiter_size, current_index - (previous_index + end_delimiter_size) );
        
        
//         std::string intial_word = readwordfromstring(section);
//         long initial_word_pos = section.find( initial_word.c_str() , 0  );
//         long final_pos =  initial_word_pos + initial_word.size();
//         while (section[final_pos] != ':')
//             final_pos++;
//         final_pos++;
//         section.substr( final_pos ,  section.size() - final_pos );

        
        
        
//         // remove first std::string after enter after readword
        
//         search_start = current_index + end_delimiter_size ;   
//     }
//     return s;  
// }


    // std::stack<string> aux = operator_stack;
    // while( !aux.empty() ) {
    //     std::cout << aux.top() << endl;
    //     aux.pop();
    // }
    // std::cout << endl;
    // aux = operand_stack;
